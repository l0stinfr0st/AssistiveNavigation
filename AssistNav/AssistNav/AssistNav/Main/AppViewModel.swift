...
final class AppViewModel: ObservableObject {
    ...
    func announceToUser(_ message: String) {
        guard let p = profile, p.spokenFeedbackEnabled else { return }
        voice.speak(message, volume: p.audioVolume, rateMultiplier: max(0.35, min(1.2, p.speechRate)))
    }
        ...
        liveNavigationTask = Task { [weak self] in
            ...
            while !Task.isCancelled {
                ...
                await MainActor.run {
                    ...
                    if let p = self.profile, p.spokenFeedbackEnabled {
                        self.voice.speak(
                            line,
                            volume: p.audioVolume,
                            rateMultiplier: max(0.35, min(1.2, p.speechRate))
                        )
                    }
                }
            }
    ...
    func submitReport(type: String, details: String) async {
        guard let user = profile else { return }
        ...
        do {
            ...
            bannerMessage = "Could not save the hazard report. Check you are signed in."
            announceToUser(bannerMessage!)
            return
        }

        bannerMessage = "Hazard report saved using your current location."
